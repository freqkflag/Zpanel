...
class CleanupHelperContainersJob implements ShouldBeEncrypted, ShouldBeUnique, ShouldQueue
{
    ...
    {
        try {
            $containers = instant_remote_process_with_timeout(['docker container ps --format \'{{json .}}\' | jq -s \'map(select(.Image | contains("'.config('constants.zpanel.registry_url').'/coollabsio/coolify-helper")))\''], $this->server, false);
